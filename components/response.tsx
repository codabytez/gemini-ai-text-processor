...
const Response: NextPage<Message> = ({ text, timestamp }) => {
  ...
  const handleTranslate = async (language: string, text: string) => {
    const res = await languageDetector(text);
    ...
    return (
      <>
        {lines.map((line, index) => (
          <p key={index} className="text-[#07090d] mb-1 break-words">
            {line.trim()}
          </p>
        ))}
      </>
  ...
  return (
    <div className="bg-[#d5dae7] rounded-2xl p-4 max-w-[300px] sm:max-w-xl lg:max-w-3xl relative">
